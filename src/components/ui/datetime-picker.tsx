      ...
      <thead {...headerProps}>
        <tr className="flex">
          {weekDays.map((day, index) => (
            <th
              className="w-9 rounded-md text-[0.8rem] font-normal dark:text-zinc-300"
              key={index}
            >
              {day}
            </th>
    ...
    >
      <Button
        ...
        className={cn(
          ...
          "h-9 w-9",
          isToday && "bg-white text-black hover:bg-white/80",
          isSelected &&
...
function DateField(props: AriaDatePickerProps<DateValue>) {
  ...
  return (
    <div
      ...
      ref={ref}
      className={cn(
        "ring-offset-background focus-visible:ring-ring inline-flex h-10 flex-1 items-center rounded-l-md border-zinc-800 bg-transparent px-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2",
...
function TimeField(props: AriaTimeFieldProps<TimeValue>) {
  ...
  return (
    <div
      ...
      ref={ref}
      className={cn(
        "ring-offset-background focus-within:ring-ring focus-visible:ring-ring inline-flex h-10 w-full flex-1 rounded-md border border-zinc-800 bg-transparent px-3 py-2 text-sm focus-within:ring-2 focus-within:ring-offset-2 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2",
        props.isDisabled && "cursor-not-allowed opacity-50"
      )}
    >
      {state.segments.map((segment, i) => (
  ...
  return (
    <div
      ...
      className={cn(
        ...
        "ring-offset-background focus-within:ring-ring flex items-center rounded-lg border dark:border-zinc-800 border-zinc-100 focus-within:ring-2 focus-within:ring-offset-2"
      )}
    >
      <Popover open={props.isOpen} onOpenChange={props.onOpenChange}>
        <PopoverTrigger asChild>
          <Button
            {...buttonProps}
            variant="ghost"
            className="rounded-r-none border-r border-[inherit]"
      ...
      <div className={cn("mr-3 size-4", !showClearButton && "hidden")}>
        <X
          className={cn(
            "size-4 cursor-pointer text-zinc-400",
            !jsDatetime && "hidden"
          )}
